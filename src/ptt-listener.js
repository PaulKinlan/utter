...
(function () {
  ...
  // Listen for offscreen recognition messages (via background)
      ...
      case 'recognition-error':
        if (!message.recoverable) {
          ...
          if (!usingOffscreenFallback && PERMISSION_ERRORS.includes(message.error)) {
            console.log('Utter PTT: Permission error in iframe, trying offscreen fallback');
  ...
  function createRecognitionFrame() {
    ...
    try {
      ...
      recognitionFrame.onerror = () => {
        console.log('Utter PTT: Iframe load error, trying offscreen fallback');
        removeRecognitionFrame();
        startOffscreenRecognition();
      };
      ...
      iframeStartupTimer = setTimeout(() => {
        if (!usingOffscreenFallback && (isKeyHeld || isRefinementKeyHeld)) {
          console.log('Utter PTT: Iframe startup timeout, trying offscreen fallback');
          removeRecognitionFrame();
          startOffscreenRecognition();
        }
  ...
  async function startOffscreenRecognition() {
    ...
    try {
      ...
      console.log('Utter PTT: Offscreen recognition started, session:', offscreenSessionId);
    } catch (err) {
      console.error('Utter PTT: Failed to start offscreen recognition:', err);
  ...
  function stopRecognition() {
    ...
    // Give iframe/offscreen a moment to send final results, then remove
    setTimeout(() => {
      removeRecognitionFrame();
    }, 100);
  }
